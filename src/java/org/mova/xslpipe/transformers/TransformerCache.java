 ...
 *
 * portions by Alexander Mikhailian <mikhailian@mova.org>
 */
...
/**
 * This caches templates that were loaded from local files so that consequent
 * calls to local stylesheets require stylesheet reparsing only if stylesheet
 * was changed.
 */
public class TransformerCache
{
	...
	/** Factory logger. */
	protected static Logger logger = Logger.getLogger(TransformerCache.class);
	...
	/** the delegating factory */
	private SAXTransformerFactory factory;

	public TransformerCache(SAXTransformerFactory factory)
	{
		this.factory = factory;
	}

	/**
	 * Process the source into an XMLFilter object. If source is a StreamSource
	 * with <code>systemID</code> pointing to a file, XMLFilter is produced from
	 * a cached templates object. Cache is done in soft references; cached objects
	 * are reloaded, when file's date of last modification changes.
	 * 
	 * @param source
	...
	public XMLFilter newXMLFilter(Source source) throws TransformerConfigurationException
	{
		...
		if (source instanceof StreamSource)
			try
			{
				...
				{
					return newXMLFilter(new File(url.getPath()));
				}
			}
			catch (MalformedURLException mue)
			{
				throw new TransformerConfigurationException(mue);
			}
			catch (TransformerConfigurationException ex)
			{
				if (factory.getErrorListener() != null)
				{
					try
					{
						factory.getErrorListener().fatalError(ex);
						return null;
					}
	...
	/**
	 * Creates a XMLFilter from a file (and caches templates) or from cached
	 * templates object.
	 * 
	 * @param file
	 *          file to load transformer from.
	 * @return XMLFilter, built from given file.
	 * @throws TransformerConfigurationException
	...
	protected XMLFilter newXMLFilter(final File file) throws TransformerConfigurationException
	{
		...
		{
			logger.debug("found " + file.getName() + " in the cache");
		...
		if (templatesCacheEntry == null)
		{
			logger.debug("Loading " + file.getAbsolutePath() + "to the cache");
			...
			templatesCacheEntry = new TemplatesCacheEntry(factory.newTemplates(new StreamSource(file)), file);
		...
		return factory.newXMLFilter(templatesCacheEntry.templates);
	}
    ...
    throws TransformerConfigurationException
  {
    ...
    if (source instanceof StreamSource)
			try
			{
				...
				if ("file".equalsIgnoreCase(url.getProtocol()))
				{
					return newTransformer(new File(url.getPath()));
				}
			}
			catch (MalformedURLException mue)
      {
